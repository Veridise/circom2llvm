...
bool isComponent(llvm::Value *v) {
  if (isa<ComponentInstance>(v)) {
    ComponentInstance *inst = dyn_cast<Constraint>(v);
    return inst->getCalledFunction()->getName().startswith_insensitive(
        fn_build_prefix);
  } else {
    return false;
  }
  ...
  for (auto &bb : F->getBasicBlockList()) {
    for (auto &inst : bb.getInstList()) {
      if (isa<Constraint>(&inst)) {
        Constraint *potential_constraint = dyn_cast<Constraint>(&inst);
        if (potential_constraint->getCalledFunction()
                ->getName()
                .startswith_insensitive(fn_constraint_prefix)) {
          this->constraints.push_back(potential_constraint);
        }
      }
    }
  }
